inp => {
    ...
    for (let KEY_star_line in tmp[0]) {
        ...
        tmp[3][KEY_star_line] ??= {}
        tmp[4][KEY_star_line] ??= {}
        tmp[5][KEY_star_line] ??= Infinity
        tmp[2][KEY_star_line] = inp['udf']['split'](';')(tmp[1][KEY_star_line][1])
        tmp[6][KEY_star_line] = inp['udf']['split'](' ')(tmp[1][KEY_star_line][0])
        for (let KEY_star_hand in tmp[2][KEY_star_line]) {
            ...
            for (let KEY_star_group in tmp[3][KEY_star_line][KEY_star_hand]) {
                ...
                tmp[5][KEY_star_line] = Math.min(tmp[5][KEY_star_line],inp['udf']['toNum'](inp['udf']['isLessOrEqual'](inp['udf']['toNum'](tmp[4][KEY_star_line][KEY_star_hand][KEY_star_group][1]),inp['bag'][tmp[4][KEY_star_line][KEY_star_hand][KEY_star_group][2]])))
            }
        }
        tmp[7][KEY_star_line] = (tmp[5][KEY_star_line]*inp['udf']['toNum'](tmp[6][KEY_star_line][1]))
    }
